...
//
$_CONFIG['calculate_space_level'] = 20;
...
// Default: $_CONFIG['users'] = array(array();
//
$_CONFIG['users'] = array();
...
$_TRANSLATIONS["cn"] = array(
	...
	"unable_to_read_dir" => "读取目录失败",
	"location" => "位置",
	"root" => "根目录",
	"log_file_permission_error" => "这个脚本没有足够权限写 log 文件",
	"upload_not_allowed" => "这个脚本配置不允许上传到这个目录",
	"upload_dir_not_writable" => "这个目录没有写入权限",
	"mobile_version" => "手机模式",
	"standard_version" => "标准模式",
	"page_load_time" => "页面读取耗时 %.2f 毫秒",
	"wrong_pass" => "错误的用户名或者密码",
	"username" => "用户名",
	"log_in" => "登录",
	"upload_type_not_allowed" => "这个文件类型不允许上传",
	"del" => "删除",
	"log_out" => "登出"
);
...
class EncodeExplorer
{
	...
	public function printLoginBox()
	{
		...
		if(GateKeeper::isLoginRequired())
		{
			...
			<div><button type="submit" class="button"><?php print $this->getString("log_in"); ?></button></div>
		</form>
		</div>
	<?php
		}
...
if(isset($_ERROR) && strlen($_ERROR) > 0)
{
	print "<div id=\"error\">".$_ERROR."</div><hr>";
}
?>
<div id="frame">
<?php
...
?>
<div id="top">
	<h1><a href="<?php print $this->makeLink(false, false, null, null, null, ""); ?>"><span><?php if(EncodeExplorer::getConfig('main_title') != null) print EncodeExplorer::getConfig('main_title'); ?></span></a></h1>
<?php
...
<thead>
  <tr>
    ...
    <th class="del"> <?php //print EncodeExplorer::getString("del"); ?></th>
    <?php } ?>
  </tr>
</thead>
<?php
...
<form enctype="multipart/form-data" method="post">
	<div id="upload">
		...
		<div id="newdir_container">
			...
			<button type="submit"><?php print $this->getString("make_directory"); ?></button>
		</div>
		<?php
		}
		...
		<div id="upload_container">
			...
			<button type="submit" class="upload_sumbit"><?php print $this->getString("upload"); ?></button>
		</div>
		<?php
		}
		?>
...
?>
<a href="#">回到顶部</a> | <a href="http://encode-explorer.siineiolekala.net">Encode Explorer</a>
</div>
<!-- END: Info area -->
</body>
</html>
